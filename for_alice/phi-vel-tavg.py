...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 21 19:07:25 2023
    ...
    for fix in fixes:
  	# Progress Check
        ...
        if stream:
    		...
    		# Specify new grid:
    		phi_num = 100
    		phis = np.linspace(-np.pi/2, np.pi/2, num = phi_num)
          		
        # EVOKE
        v_cast = THE_SMALL_CASTER(phis, PHI, Unbound_v)
...
np.save('tavg/'+pre+'yes'+name,time_averaged_phi)
np.save('tavg/'+pre+'no'+name,time_averaged_phi_stream)